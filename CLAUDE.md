...
- **CRITICAL**: Team plan commit strategy MUST match CLAUDE.md — never override with "one commit at Gate 4." If the team plan contradicts CLAUDE.md, CLAUDE.md wins. The `/agent-team-plan` template must enforce "Commit strategy: Per CLAUDE.md — commit after each phase gate"
- Clean up worktrees after merging feature branches — run `git worktree remove <path>` immediately after merge. Orphaned worktrees waste disk space and cause confusion. Add worktree cleanup to the post-merge checklist
- Transport-layer stubs (e.g., `send_room_message()` no-op) that cross UC boundaries must be tracked as explicit tech debt with a follow-up task — don't let stubs hide behind "it compiles and tests pass"
- Return-value pattern for TUI→network dispatch: `handle_command() -> Option<NetCommand>` keeps App free of channel dependencies, enables unit testing without async runtime, centralizes dispatch in main.rs. Preferred over storing channel references in App
- When App.rs exceeds ~500 lines, plan a decomposition UC (similar to UC-014 for ChatManager). Extract: command handling, conversation management, UI state. App at 976 lines with MI=-25.3 is a code health emergency
